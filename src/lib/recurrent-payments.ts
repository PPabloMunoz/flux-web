...
async function ensureInitialDataLoaded() {
  ...
  initialDataPromise = (async () => {
    try {
      const [accData, catData] = await Promise.all([fetchAccounts(), fetchCategories()]);
      accounts = accData;
      categories = catData;
      populateSelects();
    } catch (error) {
...
function populateSelects() {
  ...
  accounts.forEach((acc) => {
    ...
    option.type = 'button';
    option.className =
      'w-full text-left px-3 py-2.5 rounded-md hover:bg-[rgba(255,255,255,0.05)] transition-colors flex items-center justify-between group';
...
export function updateCategorySelect() {
  ...
  filteredCategories.forEach((cat) => {
    ...
    option.type = 'button';
    option.className =
      'w-full text-left px-3 py-2.5 rounded-md hover:bg-[rgba(255,255,255,0.05)] transition-colors flex items-center gap-3 group';
...
function renderPayments(payments: RecurrentPayment[]) {
  ...
  if (payments.length === 0) {
    rowsContainer.innerHTML =
      '<tr><td colspan="6" class="px-6 py-12 text-center text-[#8a8f98]">No recurrent payments found.</td></tr>';
    return;
  }
  ...
  payments.forEach((payment: RecurrentPayment) => {
    ...
    row.className =
      'hover:bg-[rgba(255,255,255,0.02)] transition-colors group cursor-pointer border-b border-[rgba(255,255,255,0.03)]';
